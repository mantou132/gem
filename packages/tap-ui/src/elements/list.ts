...
import { addListener, LinkedList, styleMap } from '@mantou/gem/lib/utils';

import { findScrollContainer } from '../lib/element';
import { blockContainer } from '../lib/styles';
import { once, throttle } from '../lib/timer';
import { TapResizeBaseElement } from './base/resize';
import type { VisibleBaseElement } from './base/visible';
import { TapVisibleBaseElement, visibilityObserver } from './base/visible';
...
@customElement('tap-list-outside')
@adoptedStyle(blockContainer)
export class TapOutsideElement extends TapVisibleBaseElement {}
...
export class TapListElement extends GemElement {
  ...
  #beforeItemRef = createRef<TapOutsideElement>();
  #afterItemRef = createRef<TapOutsideElement>();
  ...
  // 初次渲染
  #initLayout = (ele: TapListItemElement) => {
  ...
  #onItemResize = ({ target }: CustomEvent) => {
    const ele = target as TapListItemElement;
  ...
  #reLayoutByResize = throttle((ele: TapListItemElement) => {
    this.#initLayout(ele);
    this.#reLayout({ resize: true });
  }, 200);

  #keyElementMap = new Map<any, TapListItemElement>();
  #getElement = (key: Key) => {
    if (!this.#keyElementMap.has(key)) {
      const ele = new TapListItemElement();
      ele.setAttribute('part', TapListElement.item);
  ...
  render = () => {
    const { beforeHeight, afterHeight, renderList } = this.#state;
    return html`
      <slot name=${TapListElement.before}></slot>
      <tap-list-outside
        ${this.#beforeItemRef}
        v-if=${this.infinite}
        part=${TapListElement.beforeOutside}
        .intersectionRoot=${this.scrollContainer}
        @show=${this.#onBeforeItemVisible}
        style=${styleMap({ height: `${beforeHeight}px` })}
      ></tap-list-outside>
      <div ${this.#listRef} class="list" part=${TapListElement.list}>
        ${
          this.infinite
      ...
      <tap-list-outside
        ${this.#afterItemRef}
        part=${TapListElement.afterOutside}
        .intersectionRoot=${this.scrollContainer}
        @show=${this.#onAfterItemVisible}
        style=${styleMap({ height: `${afterHeight}px` })}
      >
      </tap-list-outside>
      <slot name=${TapListElement.after}>
        <!-- 无限滚动时避免找不到 "tap-list-outside", e.g: tap-list docs -->
...
@customElement('tap-list-item')
@adoptedStyle(blockContainer)
@aria({ role: 'listitem' })
// Chrome bug: https://x.com/594mantou/status/1944406907319661026
@light({ penetrable: true })
export class TapListItemElement extends TapResizeBaseElement implements VisibleBaseElement {
  ...
  render = () => {
    ...
    return this.renderItem ? this.renderItem(this.item) : this.item;
  };
}

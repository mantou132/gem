...
import type { TapOptionsElement } from './options';
...
export type ContextMenuOptions = MenuOptions & {
...
@customElement('tap-contextmenu')
@connectStore(contextmenuStore)
@adoptedStyle(style)
@shadow({ delegatesFocus: true })
export class TapContextmenuElement extends GemElement {
  static instance?: TapContextmenuElement;
  ...
  #optionsRef = createRef<TapOptionsElement>();
  ...
  #execHandle = (handle?: () => void) => {
    if (handle) {
      (this.constructor as typeof TapContextmenuElement).close();
      handle();
    }
  };
  ...
  #onKeydown = (evt: KeyboardEvent, menuStackIndex: number) => {
    evt.stopPropagation();
    const Ctor = this.constructor as typeof TapContextmenuElement;
    hotkeys({
      esc:
        menuStackIndex === 0
          ? () => Ctor.close()
              ...
              Array.isArray(menu)
                ...
                : html`
                  <div class="menu-custom-container">
                    <tap-compartment .content=${menu}></tap-compartment>
                  </div>
                `
            }
          </tap-options>
...
export const ContextMenu = TapContextmenuElement;

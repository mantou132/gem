...
@customElement('tap-countdown')
@adoptedStyle(style)
@aria({ role: 'timer' })
@shadow()
export class TapCountdownElement extends GemElement {
  ...
  render = () => {
    ...
    return html`
      <div part=${TapCountdownElement.hours}>${this.#fill(hours)}</div>
      <div part=${TapCountdownElement.delimiter} class="delimiter"></div>
      <div part=${TapCountdownElement.minutes}>${this.#fill(minutes)}</div>
      <div part=${TapCountdownElement.delimiter} class="delimiter"></div>
      <div part=${TapCountdownElement.seconds}>${this.#fill(seconds)}</div>
    `;
  };
}

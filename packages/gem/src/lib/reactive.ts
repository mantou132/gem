...
export let render = ((value: string, container: HTMLElement | ShadowRoot) =>
  (container.innerHTML = value)) as unknown as typeof Lit.render;

export function setEngine(h: typeof Lit.html, r: typeof Lit.render) {
  html = h;
  render = r;
}
...
export type Sheet<T> = { [P in keyof T]: string } & { [SheetToken]: GemCSSSheet };
...
export abstract class GemElement extends HTMLElement {
  ...
  /** @lifecycle 将来会彻底移除，现在做兼容实现*/ render?(): Lit.TemplateResult | null | undefined;
  #compat = () => {

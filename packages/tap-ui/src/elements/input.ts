...
class InputHistory {
  current: number;
  values: string[];
  #input: TapInputElement;

  constructor(input: TapInputElement) {
    this.#input = input;
    this.current = 0;
    this.values = [''];
  }
...
export class TapInputElement extends GemElement {
...
const inputGroupStyle = css`
  :scope {
    ...
    :where(tap-input, tap-select, tap-picker):where(:focus, :focus-within, :hover, :state(active)) {
      position: relative;
      z-index: 1;
    }
    > * {
    ...
    > :nth-child(n + 2),
    /* > tap-tooltip > tap-button */
    > tap-tooltip:last-child > * {
...
@customElement('tap-input-group')
@adoptedStyle(inputGroupStyle)
@aria({ role: 'group' })
export class TapInputGroupElement extends GemElement {}

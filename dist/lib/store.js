...
export function updateStore(storeModule, value) {
    if (!updaterSet.size) {
...
export function connect(storeModule, func) {
    const storeKey = storeModule[STORE_MODULE_KEY];
    if (!storeKey)
        throw new Error('Parameter error');
    const listeners = storeModule[HANDLES_KEY].get(storeKey);
    if (!func[FUNC_MARK_KEY])
        func[FUNC_MARK_KEY] = new Set();
    func[FUNC_MARK_KEY].add(storeKey);
    listeners.add(func);
}
export function disconnect(storeModule, func) {
    const storeKey = storeModule[STORE_MODULE_KEY];
    if (!storeKey)
        throw new Error('Parameter error');
    const listeners = storeModule[HANDLES_KEY].get(storeKey);

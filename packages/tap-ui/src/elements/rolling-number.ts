...
@customElement('tap-rolling-number')
@adoptedStyle(style)
@shadow()
export class TapRollingNumberElement extends GemElement {
    ...
    return html`
      ${items.map((item) => {
        if (item.kind === Kind.Separator) {
          return html`<span part=${TapRollingNumberElement.separator} class="separator">${item.char}</span>`;
        }
        ...
        if (!isAnimating || item.fromOffset === item.toOffset) {
          return html`
            <span part=${TapRollingNumberElement.digit} class="digit-container">

...
export const templates = (value, onClick, ref, items) => [
  ...
  html`
    <button
      title="before ${value} >   < <!-- literal --> after"
      data-value=${value}
      .value=${value}
      ?disabled=${false}
      @click=${onClick}
  ...
  html`
    <span>a</span>   <span>b</span> ${value} <b>c</b>
  `,
  html`
    <pre><code>first</code>
    <code>${value}</code></pre>
  ...
  html`
    <textarea><!-- literal --> >   < ${value} >   <</textarea>
  `,
  html`
    <title>before <!-- literal --> ${value} after</title>
  `,
  html`
    <style>
      .item::before {
  ...
  html`
    <div v-if=${value} ${ref} ${{ title: value }}>${value}</div>
    <span v-else>empty</span>
  `,
  // SVG text whitespace and namespaced attributes.
  html`
    <svg>
      <text xml:space="preserve"><tspan>a</tspan>   <tspan>${value}</tspan></text>
    </svg>
  `,
  svg`
    <circle
